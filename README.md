...
Je dois de nouveau changer le langage de mon clavier avec sudo cette fois. Pour avoir un clavier AZERTY dès le lancement de ma machine je tape la commande suivante: echo KEYMAP=fr > /etc/vconsole.conf qui créé ce fichier de configuration du sytème qui sera lu par systemd dès le lancement.
